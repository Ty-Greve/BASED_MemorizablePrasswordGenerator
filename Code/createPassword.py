def createPassword(userAnswers, passwordLength):
  ...
  for i in range(random.randrange(3)):
    ...
    if (diceRoll == 1):
      print(1)
      userAnswers[i] = randomObj.findAnagram(userAnswers[i])
    elif (diceRoll == 2):
      print(2)  
...
#Check the upper and lower boundaries with the mega string, and selects the appropiate original word 
  ...
  elif len(userAnswers[0])< lower < len(userAnswers[0]) + len(userAnswers[1]):   #If lower bound is between the first and second words, that means the second word was in
